...
const MenuCart = (cart) => {
    ...
    const GenerateMenuCart = (item) => {
        const menuItem = item.data.card.info;
        return (
            <>
                                ...
                                <div className="flex justify-center flex-col items-center ">
                                    <div className="bg-white border-1 hover:bg-gray-300 ease-in-out duration-200 transition-all border-solid h-10 rounded-lg w-[120px] relative overflow-hidden inline-flex items-center justify-between shadow-lg">
                                        <div className="absolute inset-0">
                                            <button className="absolute w-32 inset-0 text-center transition-all duration-100 translate-y-0
    ...
    return (
        <div className="ml-5">
            {subMenu.map(item => <GenerateMenuCart data={item} key={item.card.info.id} />)}
        </div>
    )
}
